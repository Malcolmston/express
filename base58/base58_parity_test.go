...
// Upstream-parity tests for the Bitcoin Base58 encoding.
//
// Vectors are transcribed verbatim from the canonical test fixtures of the
// reference implementation this package ports, cryptocoinjs/base-x:
//
//	https://raw.githubusercontent.com/cryptocoinjs/base-x/master/test/fixtures.json
//
// Only the entries with "alphabet": "base58" (alphabet
// "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz", identical to
// this package's alphabet) are used. Each valid vector is a hex string paired
// with its Base58 encoding; the invalid vectors are strings base-x rejects
// with "Non-base58 character".
...
// base58Vectors are the {hex, base58} pairs from fixtures.json (valid, base58).
...
// TestParityEncode checks Encode against every base58 vector in fixtures.json.
func TestParityEncode(t *testing.T) {
	for _, tt := range base58Vectors {
		raw, err := hex.DecodeString(tt.hex)
		if err != nil {
			t.Fatalf("bad hex fixture %q: %v", tt.hex, err)
		}
		if got := Encode(raw); got != tt.b58 {
			t.Errorf("Encode(%s) = %q, want %q", tt.hex, got, tt.b58)
		}
	}
}

// TestParityDecode checks Decode against every base58 vector in fixtures.json.
func TestParityDecode(t *testing.T) {
	for _, tt := range base58Vectors {
		wantHex := tt.hex
		got, err := Decode(tt.b58)
		if err != nil {
			t.Errorf("Decode(%q): unexpected error %v", tt.b58, err)
			continue
		}
		if gotHex := hex.EncodeToString(got); gotHex != wantHex {
			t.Errorf("Decode(%q) = %s, want %s", tt.b58, gotHex, wantHex)
		}
	}
}

// invalidStrings are the base58 inputs that base-x rejects as containing a
// "Non-base58 character".
var invalidStrings = []string{
	"#####",
	"invalid",                       // 'l' is not in the alphabet
	"c2F0b3NoaQo=",                  // base64, contains '=' and '0'
	" 1111111111",                   // leading whitespace
	"1111111111 ",                   // trailing whitespace
	" \t\n\v\f\r skip \r\f\v\n\t a", // interior whitespace
	"က",                             // rune whose code point exceeds the map
}

// TestParityInvalid confirms Decode rejects the fixtures.json invalid strings.
func TestParityInvalid(t *testing.T) {
	for _, s := range invalidStrings {
		if _, err := Decode(s); err != ErrInvalidCharacter {
