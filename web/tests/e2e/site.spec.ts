...
// and served at /express/api/; it is not present in the plain `vite build`
...
test('internal jump links land on an existing section on the overview', async ({ page }) => {
  await gotoTab(page, 'overview');
  // The "on this page" nav links to in-page section ids like #install.
  await page.locator('.view.active a[href="#install"]').first().click();
